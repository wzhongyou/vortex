...
- **Block 编码** — 128-doc 块位压缩倒排链，可选 AVX2 加速，多层跳表
...
├──────────────────────────────────┤
│ FST Dict │ BlockCodec │ SkipList│  ← 高性能编码
├──────────────────────────────────┤
...
搜索引擎风格的 Web 搜索页面，浏览器中直接体验 Vortex 的搜索能力：

![搜索演示截图](docs/images/search_demo.png)
...
```json
[
  {"title": "文档标题", "content": "文档内容", "category": "分类"},
  ...
]
```
...
| ------------------------------- | ---- | --------------------- |
| `/`                           | GET  | 搜索页面              |
| `/api/search?q=关键词&page=1` | GET  | 搜索（JSON）          |
| `/api/document/:id`           | GET  | 文档详情（JSON）      |
| `/api/document`               | POST | 添加文档（JSON body） |
...
```json
{
  ...
  "elapsed_ms": 0.12,
  "results": [
    {"id": "2", "title": "Introduction to Python", "url": "https://docs.python.org/3/tutorial/", "site": "docs.python.org", "description": "Official Python tutorial for beginners", "category": "Programming", "score": 2.78}
  ]
}
```

完整源码见 [examples/search_demo/](examples/search_demo/)。
...
```cpp
Schema schema;
schema.add_field({"title",       FieldType::TEXT,    true,  true});   // 分词索引 + 存储
schema.add_field({"content",     FieldType::TEXT,    true,  true});   // 分词索引 + 存储
schema.add_field({"url",         FieldType::KEYWORD, true,  false});  // 精确匹配，不存储
schema.add_field({"site",        FieldType::KEYWORD, true,  false});  // 精确匹配，不存储
schema.add_field({"author",      FieldType::TEXT,    true,  true});   // 分词索引 + 存储
schema.add_field({"timestamp",   FieldType::KEYWORD, true,  false});  // 精确匹配，不存储
schema.add_field({"description", FieldType::TEXT,    true,  true});   // 分词索引 + 存储
schema.add_field({"doc_id",      FieldType::KEYWORD, true,  false});  // 文档唯一标识
schema.add_field({"category",    FieldType::KEYWORD, true,  false});  // 精确匹配，不存储
```
...
doc.fields.push_back({"url",         "https://github.com/wzhongyou/vortex"});
doc.fields.push_back({"site",        "github.com"});
doc.fields.push_back({"author",      "wzhongyou"});
doc.fields.push_back({"timestamp",   "2026-06-01"});
doc.fields.push_back({"description", "A fast C++17 inverted index engine"});
...
| `VORTEX_USE_AVX2`         | x86_64 自动开启 | 编译 AVX2 版块解码（当前未运行时接入）      |
| `VORTEX_USE_ICU`          | OFF             | ICU 国际化分词                              |

## 设计原则

1. **数据结构决定性能上限** — FST、BlockCodec、SkipList、Roaring，每个选择都有行业基准验证
...
- [倒排索引 V1 设计](docs/inverted_index_design.md) — 段架构、FST、BlockCodec、WAL、BM25F、RCU 并发模型
- [搜索演示设计](docs/search_demo_design.md) — Web 搜索页面架构、CJK 查询、REST API
- [索引构建协议](docs/index_protocol.md) — 段文件二进制格式、WAL 协议、REST API、Schema 协议（面向外部系统集成）
